...
public class RevistaCrudController {
	...
	@ResponseBody
	public ResponseEntity<?> listaRevistaPorNombreLike(@PathVariable("var") String nombre){
		List<Revista> lstSalida = null;
		if (nombre.equals("todos")) {
			lstSalida = revistaService.listaTodos();
		}else {
			lstSalida = revistaService.listaRevistaPorNombreLike(nombre +  "%");
		}
